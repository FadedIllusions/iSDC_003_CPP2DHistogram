/*
   ...
   Purpose: helper functions which are useful when implementing 
   a 2-dimensional histogram filter.
*/
...
/*
    ...
    where each entry represents the unnormalized probability associated
    with that grid cell.
...
{
   //Pass by ref, negating need for 'newGrid' vec and extra mem alloc
   vector< vector<float> > newGrid;
   float total = 0.0;

   // Iterate through grid so as to calculate total
   ...
   // Using total, create grid of normalized values
   for(int i=0; i<grid.size(); i++)
   {
      // Recurring v<f> vs mem? Embedded sys. Hmm...
      vector<float> row;
      row.clear();
...
/*
   ...
   which represents how much probability from one cell "spills over" 
   to it's neighbors. If it's 0.0, then no blurring occurs. 
...
vector< vector<float> > blur(vector< vector<float> > grid, float blurring)
{
   ...
   float center, corner, adjacent, gridVal, multiplier;
   int newI, newJ;
      ...
      for(int j=0; j<width; j++)
      {
         ...
         for(int dx=-1; dx<2; dx++)
         {
            for(int dy=-1; dy<2; dy++)
            {
               ...
               newJ = (j+dx+height)%width;
               newGrid[newI][newJ] += multiplier*gridVal;
            }
         }
      }
   }
